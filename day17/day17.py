...
d = 'UDLR'
direction_mapper = dict(D=(0, -1), U=(0, 1), L=(-1, 0), R=(1, 0))
...
def new_coords(instruction, x, y):
    a, b = direction_mapper[instruction]
    return (x + a, y + b)


def check_hash(path, xcoord, ycoord):

    #print hashlib.md5(path).hexdigest()[:4]
    hexadiga = [x in 'bcdef' for x in hashlib.md5(path).hexdigest()[:4]]

    directions_master = dict(zip(d,hexadiga))
    directions = {k: v for k, v in directions_master.iteritems() if v}

    forbiddan_dir = whereami(xcoord,ycoord)

    for x in forbiddan_dir:
        try:
            del directions[x]
        except:
            pass
...
def main(path, coords, goal):
    dqe = deque()

    xcoord, ycoord = coords

    dqe.append((coords, path, ''))
    finalpath = []
    while dqe:

        (xcoord, ycoord), path, directions = dqe.popleft()

        for direction in check_hash(path, xcoord, ycoord):
            nxcoord, nycoord = new_coords(direction, xcoord, ycoord )

            if (nxcoord, nycoord) == goal:
                finalpath.append(directions + direction)

            else:
                # add more coords
                dqe.append(((nxcoord, nycoord), path+direction, directions + direction))
...
main('ihgpwlah', start, goal)
main('kglvqrro', start, goal)
main('ulqzkmiv', start, goal)

puzzle_input = "qtetzkpl"
print "Puzzle outputs.."
main('qtetzkpl', start, goal)  # RRRLDRDUDD
