...
at time=100, then the capsule reaches the top disc at time=101, the second disc
...
it would reach the first disc at time=1. Since the first disc was at position 4
...
input1 = """Disc #1 has 17 positions; at time=0, it is at position 1.
...
input2 = input1 + "Disc #7 has 11 positions; at time=0, it is at position 0."

tc1 = """Disc #1 has 5 positions; at time=0, it is at position 4.
Disc #2 has 2 positions; at time=0, it is at position 1.
"""
...
def generate_pairs(input):
    return [(re.search(regex, x).groups()) for x in input.splitlines()]


def populate(offset, pos):
    return [(a == (offset % pos)) for a in range(pos)]


def rotate(ls):
    return ls.insert(0, ls.pop())


def load_pairs(inp):
    return \
        [(populate(int(x[1]) + (i + 1), int(x[0]))) for i, x in enumerate(inp)]


def main(input):
    pairs = generate_pairs(input)
    ls = load_pairs(pairs)

    time = 0

    while 1:
        if all(reduce(lambda x, y: x + y, zip(x[0] for x in ls))):
            return time
        [rotate(x) for x in ls]
        time += 1


print main(tc1)  # 5
print main(input1)  # 317371
print main(input2)  # 2080951
