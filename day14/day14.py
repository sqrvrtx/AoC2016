...
def create_hex(salt, stretched=False):

    if stretched:
        tmp_salt = salt
        for i in xrange(2017):
            tmp_salt = md5(tmp_salt).hexdigest()

        hash_dict[tmp_salt] = tmp_salt
        return tmp_salt
    else:
        hash_dict[salt] = md5(salt).hexdigest()
        return hash_dict[salt]
import re
regex3 = re.compile(r'(\w)\1\1')
regex5 = re.compile(r'(\w)\1\1')

# def isMultiple(s, mult, letter=None):
#     return re.search(regex, s)
#     # for i in range(len(s)-(mult-1)):
#     #     if all((s[i] == s[i+y]) for y in range(1, mult)):
#     #         if letter:
#     #             if s[i] == letter:
#     #                 return s[i]
#     #         else:
...
def is_triple(input_str):
    result = re.search(regex3, input_str)
    if result:
        return result.groups()[0]


def find_quintuple(input_str, letter):
    return letter if letter*5 in input_str else None



def main(input_str, stretched=False):
    count64 = 1
    count = 0
    while count64 <= 64:
        salt = input_str + str(count)
        hexdig = hash_dict.get(salt, create_hex(salt, stretched))

        letter = is_triple(hexdig)
        if letter:
            # Find quintuplet
            #if any(find_quintuple(hash_dict.get(input_str + str(count + x),

            for x in xrange(1, 1001):  # noqa
                if find_quintuple(hash_dict.get(input_str + str(count + x), create_hex(input_str + str(count + x), stretched)), letter):
                    print hexdig, count64, count
                    count64 += 1
                    break
        count += 1
    return count -1
s = '3aeeeee1367614f3061d165a5fe3cac3'
s_false = '3aevbd1367614f3061d165a5fe3cac3'
assert is_triple(s), is_triple(s)
assert find_quintuple(s, 'e')

print is_triple(s_false), is_triple(s_false)
assert not find_quintuple(s_false, 'e')

# Part i)
# assert  main(input1) == 22728
# print "#"*80
# assert main(input2) == 18626
# print "#"*80
# # Part ii)
# hash_dict = {}  # reset
# main(input1, stretched=True)  # 22551

hash_dict = {}  # reset
main(input2, stretched=True)  # ??
