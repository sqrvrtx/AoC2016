...
so visitors can behold the many fancy conference rooms and water coolers on this
...
s = """
...
pad = {'R': [1,rr], 'U': [-3,uu], 'D': [3,dd], 'L': [-1,ll]}

f1 = lambda x,y: x+pad[y][0] if x not in pad[y][1] else x

print [reduce(f1, z, 5) for z in s.splitlines()]
...
assert num_padder(test_case1) == '5DB3'

print num_padder(s) # A6B35
