...
How many viable pairs of nodes are there? # 1024
...
ls = []
Disk = namedtuple('Disk', ['x', 'y', 'sized','used','avail','perc'] )

for line in lines:
    groups = [x for x in map(int, re.findall(r'(\d+)', line))]
    ls.append(Disk._make(groups))

check_valid = lambda x,y: x.used < y.avail and x.used != 0
sum_nodes = sum((check_valid(x, y) for x, y in permutations(ls, 2))) # noqa 1024
