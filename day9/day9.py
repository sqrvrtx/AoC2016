...
For example:
    ...
    (for a decompressed length of 18), because the decompressed data from the
...
def decompress(data, recursive=False):
    ...
    while "(" in data:
        ...
        letter_indx, multiplier = map(int, data[1:indx_r].split('x'))

        # Remove the chunk
        data = data[indx_r+1:]

        if not recursive:
            count += len(data[:letter_indx] * multiplier)

        else:
            count += decompress(data[:letter_indx], recursive=True) * multiplier
        data = data[letter_indx:]
...
assert decompress('ADVENT') == 6 #'ADVENT'
assert decompress('A(1x5)BC') == 7 #'ABBBBBC', decompress('A(1x5)BC')
assert decompress('(3x3)XYZ') == 9 # becomes XYZXYZXYZ for a decompressed length of 9. # 9
assert decompress('A(2x2)BCD(2x2)EFG') == 11 #'ABCBCDEFEFG'
assert decompress('(6x1)(1x3)A') ==  6 #'|1x3|A' #11, decompress('(6x1)(1x3)A')# 6)
assert decompress('X(8x2)(3x3)ABCY') == 18 #'X|3x3|ABC|3x3|ABCY', decompress('X(8x2)(3x3)ABCY')


if __name__ == '__main__':
    with open('9.in') as f:
        data = f.read()
        print("Part 1:", decompress(data)) # 97714
        print("Part 2:", decompress(data, recursive=True)) # 10762972461
